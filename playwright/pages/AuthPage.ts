...
import { DemoRole } from "../test-data/types";
...
export class AuthPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.applicationContainer = page.locator("#app");

    this.customerLoginButton = page.locator(
      'button.enter[data-role="customer"]'
    );

    this.adminLoginButton = page.locator(
      'button.enter[data-role="admin"]'
    );

    this.currentUser = page.locator("#who");
    this.logoutButton = page.locator("#logout");

    this.dashboardSection = page.locator("#dashboard");
    this.adminNavigationButton = page.locator("#adminNav");
    this.adminSection = page.locator("#admin");
  }
  ...
  async expectLoginScreenVisible(): Promise<void> {
    ...
    await expect(this.adminLoginButton).toBeVisible();

    await expect(this.applicationContainer).toBeHidden();
  }
  ...
  async loginAs(role: DemoRole): Promise<void> {
    ...
    await expect(this.applicationContainer).toBeVisible();
    await expect(this.loginContainer).toBeHidden();

    await expect(this.dashboardSection).toBeVisible();

    await expect(this.currentUser).toContainText(
      new RegExp(role, "i")
    );
  }
  ...
  async expectCustomerAccess(): Promise<void> {
    await expect(this.currentUser).toContainText(/customer/i);

    await expect(this.adminNavigationButton).toBeHidden();

    await expect(this.adminSection).toBeHidden();
  }

  async expectAdminAccess(): Promise<void> {
    await expect(this.currentUser).toContainText(/admin/i);

    await expect(this.adminNavigationButton).toBeVisible();
  }

  async openAdminConsole(): Promise<void> {
    await this.adminNavigationButton.click();

    await expect(this.adminSection).toBeVisible();

    await expect(
      this.adminSection.getByRole("heading", {
        name: "Admin console"
      })
    ).toBeVisible();
  }
  ...
  async expectSessionStorageCreated(): Promise<void> {
  ...
  async expectSessionStorageCleared(): Promise<void> {
    const session = await this.page.evaluate(() => ({
      token: sessionStorage.getItem("nb_token"),
      user: sessionStorage.getItem("nb_user")
    }));
